package com.wemolian.app.zxing.encode;
...
//	}
	 ...
	 * @return
	 */
	public static Bitmap createQRCode(String str,Bitmap inbitmap) {
		// 生成二维矩阵,编码时指定大小,不要生成了图片以后再进行缩放,这样会模糊导致识别失败
		try {
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					if (x > halfW - IMAGE_HALFWIDTH && x < halfW + IMAGE_HALFWIDTH && y > halfH - IMAGE_HALFWIDTH
						...
						pixels[y * width + x] = inbitmap.getPixel(x - halfW + IMAGE_HALFWIDTH, y
