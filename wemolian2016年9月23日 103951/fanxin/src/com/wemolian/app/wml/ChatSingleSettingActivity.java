...
public class ChatSingleSettingActivity extends BaseActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Contacts user = WeMoLianApplication.getInstance().getContactList().get(userId);
        // 资料错误则不显示
        if (user == null) {
            return;
        }
        userNick = user.getNick();
        avatar = user.getImgName();
//        sex = user.getSex();
        // 黑名单列表
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            progressDialog.dismiss();
                            Toast.makeText(ChatSingleSettingActivity.this,
                                    "设置失败，原因：" + e.toString(),
        ...
        try {
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    Toast.makeText(ChatSingleSettingActivity.this, "设置失败",
                            Toast.LENGTH_SHORT).show();
                }
            });
        }
