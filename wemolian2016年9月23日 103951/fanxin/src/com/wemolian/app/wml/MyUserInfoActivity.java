...
public class MyUserInfoActivity extends BaseActivity {
    ...
    private RelativeLayout re_user_ename;
...
//    private TextView tv_address;
    ...
    private static final int PROFILE_UPDATE_REGION = 10;// 结果
    ...
    private void initView() {
        userEName = LocalUserInfo.getInstance(MyUserInfoActivity.this).getUserInfo(
                LocalDBKey.USER_COLUMN_NAME_USERENAME);
        userCName = LocalUserInfo.getInstance(MyUserInfoActivity.this).getUserInfo(
        		LocalDBKey.USER_COLUMN_NAME_USERCNAME);
        externalUse = LocalUserInfo.getInstance(MyUserInfoActivity.this).getUserInfo(
        		LocalDBKey.USER_COLUMN_NAME_EXTERNALUSE);
        sex = LocalUserInfo.getInstance(MyUserInfoActivity.this).getUserInfo(
        		LocalDBKey.USER_COLUMN_NAME_USERSEX);
        autograph = LocalUserInfo.getInstance(MyUserInfoActivity.this).getUserInfo(
        		LocalDBKey.USER_COLUMN_NAME_AUTOGRAPH);
        avatar = LocalUserInfo.getInstance(MyUserInfoActivity.this)
                .getUserInfo(LocalDBKey.USER_COLUMN_NAME_HEADIMG);
        address = LocalUserInfo.getInstance(MyUserInfoActivity.this)
        		.getUserInfo(LocalDBKey.USER_COLUMN_NAME_USERADDRESS);
        userRegion = LocalUserInfo.getInstance(MyUserInfoActivity.this)
        ...
        re_user_ename = (RelativeLayout) this.findViewById(R.id.re_user_ename);
        re_sex = (RelativeLayout) this.findViewById(R.id.re_sex);
        re_region = (RelativeLayout) this.findViewById(R.id.re_region);
        re_autograph = (RelativeLayout) this.findViewById(R.id.re_autograph);
        ...
        re_user_ename.setOnClickListener(new MyListener());
        re_sex.setOnClickListener(new MyListener());
        re_region.setOnClickListener(new MyListener());
        re_autograph.setOnClickListener(new MyListener());
        re_qrcode.setOnClickListener(new MyListener());
        // 头像
...
//        tv_address = (TextView) this.findViewById(R.id.tv_address);
        if(userEName == null){
        	tv_user_ename.setText("未设置");
        }else{
        	tv_user_ename.setText(userEName);
        }
        if( userCName == null){
        	 tv_name.setText("未设置");
        }else{
        ...
        if(userRegion == null){
        	tv_region.setTag("未设置");
        }else{
        	tv_region.setText(userRegion);
        }
//        if(address == null){
//        	tv_address.setText("未设置");
//        }else{
//        	tv_address.setText(address);
//        }
        if (autograph.equals("0")) {
...
//                break;
            ...
            case R.id.re_region:
            	...
            	startActivityForResult(new Intent(MyUserInfoActivity.this,
                        UpdateRegionActivity.class)
            	.putExtra("county", "china")
            	.putExtra("region", userRegion),PROFILE_UPDATE_REGION);
                break;
                //个性签名
            case R.id.re_autograph:
            	
            	startActivityForResult(new Intent(MyUserInfoActivity.this,
                        UpdateAutographActivity.class)
            	.putExtra("externalUse", externalUse)
            	.putExtra("userEName", userEName)
            	.putExtra("autograph", autograph),UPDATE_NICK);
                break;
            	//二维码
            case R.id.re_qrcode:
            	startActivityForResult(new Intent(MyUserInfoActivity.this,
            	...
            	.putExtra("externalUse", externalUse)
            	.putExtra("region", userRegion),UPDATE_NICK);
            	break;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            ...
            case PROFILE_UPDATE_REGION:
            	String newRegion = data.getExtras().getString("result");
            	tv_region.setText(newRegion);
    ...
    private void updateAvatarInServer(final String image) {
        ...
        if ((new File("/sdcard/wemolian/" + image)).exists()) {
            map.put("file", "/sdcard/wemolian/" + image);
            map.put("image", image);
        } else {
            return;
        }
        map.put("hxid", externalUse);
